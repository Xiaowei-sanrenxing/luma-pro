...
const ETHNIC_FEATURES = {
    ...
    child_caucasian: {
        general: 'Adorable European/Caucasian child (age 3-12) with sweet innocent features: bright sparkling eyes, soft fair skin, small delicate facial features, child proportions',
        specific: 'Round cherubic face, small button nose, natural pink cheeks, soft hair (blonde/brown/red variations), sweet innocent expression, petite child frame, joyful and pure demeanor, appropriate for flower girl or ring bearer roles'
    }
};
...
const PRESET_MODELS = [
    ...
    },
    {
        id: 'me_1',
        label: '中东经典',
        src: 'https://images.unsplash.com/photo-1531746020798-e6953c6e8e04?w=200&h=200&fit=crop&q=80',
    ...
    },
    {
        ...
        desc: 'ELEGANT Middle Eastern woman with REFINED features: GRACEFUL prominent aquiline nose, BEAUTIFULLY arched thick eyebrows, MESMERIZING deep-set dark almond eyes, WARM olive complexion, ELEGANT high cheekbones, SOPHISTICATED bone structure, LUXURIOUS long wavy dark hair, TIMELESS Middle Eastern beauty'
    },
    {
        id: 'me_4',
        label: '中东甜美',
        src: 'https://images.unsplash.com/photo-1502823403499-6ccfcf4fb453?w=200&h=200&fit=crop&q=80',
        desc: 'SWEET Middle Eastern woman with YOUTHFUL features: CHARMING aquiline nose, EXPRESSIVE thick dark eyebrows, WARM inviting almond eyes, GLOWING bronze skin, SOFT high cheekbones, GENTLE smile, FLOWING dark wavy hair, APPROACHABLE and friendly Arab beauty'
    },
    {
        id: 'me_5',
        label: '中东高贵',
        src: 'https://images.unsplash.com/photo-1524502397800-2eeaad7c3fe5?w=200&h=200&fit=crop&q=80',
        desc: 'REGAL Middle Eastern woman with ARISTOCRATIC features: COMMANDING aquiline nose, PERFECTLY sculpted thick eyebrows, PIERCING deep almond eyes, RICH dark olive skin, STRIKING angular cheekbones, DEFINED jawline, MAJESTIC presence, VOLUMINOUS jet-black hair - ROYAL Arab elegance'
    },
    {
        id: 'child_eu_girl',
        label: '欧美女童',
        src: 'https://images.unsplash.com/photo-1503454537195-1dcabb73ffb9?w=200&h=200&fit=crop&q=80',
        desc: 'ADORABLE European/Caucasian little girl (age 3-12) with SWEET features: bright sparkling eyes (blue/green/hazel), soft fair skin with natural rosy cheeks, small delicate button nose, innocent sweet smile, soft flowing hair (blonde/light brown), CHARMING flower girl appearance, perfect for wedding accessories like mini veils and headpieces'
    },
    {
        id: 'child_eu_boy',
        label: '欧美男童',
        src: 'https://images.unsplash.com/photo-1503919545889-aef636e10ad4?w=200&h=200&fit=crop&q=80',
        desc: 'HANDSOME European/Caucasian little boy (age 3-12) with CHARMING features: bright curious eyes (blue/green/brown), fair skin, neat short hair, small well-proportioned features, sweet genuine expression, ADORABLE ring bearer appearance, dressed elegantly for wedding ceremonies'
    },
];
...
const ETHNIC_NEGATIVE_PROMPTS = {
    ...
    child_caucasian: 'adult, mature, teenager, deep wrinkles, facial hair, mature body proportions, heavy makeup, aged appearance, sensual, sexy, provocative'
};
...
const POSE_CATEGORIES = [
    ...
    { id: 'back_view', label: '背面展示', icon: UserCog },
...
const POSE_PRESETS: Record<string, { id: string, label: string, icon: any, color: string, prompt: string, desc: string }[]> = {
    ...
    'back_view': [ // 背面展示姿势 - 新增
        { id: 'back_straight', label: '纯正背面', icon: UserCog, color: '#F0F9FF', desc: '背部全景', prompt: 'Model standing with back completely facing camera, straight posture, arms naturally at sides, showcasing full back view of outfit and accessories, clean symmetrical composition, studio lighting highlighting back details.' },
        { id: 'back_turn_head', label: '背影回眸', icon: UserCog, color: '#FDF2F8', desc: '回头看镜头', prompt: 'Model standing with back to camera, elegantly turning head over shoulder to look back at camera, showing both back design and partial face profile, graceful neck line, romantic atmosphere.' },
        { id: 'back_45_angle', label: '背部45度', icon: UserCog, color: '#E0E7FF', desc: '斜侧背面', prompt: 'Model positioned at 45-degree angle with back towards camera, showing side profile and back simultaneously, dynamic composition, highlighting shoulder and back accessories.' },
        { id: 'back_over_shoulder', label: '越肩回望', icon: UserCog, color: '#FCE7F3', desc: '肩部特写', prompt: 'Close-up shot over model shoulder from behind, focusing on neck, shoulder area and upper back, perfect for showcasing veils, necklaces, and back jewelry, soft bokeh background.' },
        { id: 'back_hair_lift', label: '撩发露背', icon: UserCog, color: '#FEF3C7', desc: '手撩头发', prompt: 'Model from behind, lifting hair with one or both hands to reveal bare back and neck, elegant gesture showing off back details of dress or accessories, sensual and sophisticated.' },
        { id: 'back_walking_away', label: '渐行渐远', icon: Footprints, color: '#DCFCE7', desc: '动态背影', prompt: 'Model walking away from camera, mid-stride back view, flowing dress or veil trailing behind, hair and fabric in motion, cinematic atmosphere, romantic departure scene.' },
        { id: 'back_sit_show', label: '坐姿背面', icon: Armchair, color: '#FFFBEB', desc: '坐着露背', prompt: 'Model sitting with back to camera, elegant posture showing backless dress or back accessories, graceful spine line visible, soft lighting on back, sophisticated studio portrait.' },
        { id: 'back_low_angle', label: '仰拍背影', icon: Camera, color: '#EEF2FF', desc: '低角度背面', prompt: 'Low angle shot from behind, looking up at model back, dramatic perspective making figure appear taller and more majestic, veil or train flowing down, powerful and elegant.' },
        { id: 'back_mirror', label: '镜中背影', icon: Box, color: '#F3E8FF', desc: '镜子反射', prompt: 'Model back to camera but reflected in a mirror, showing both back view directly and front reflection, artistic composition, perfect for showing complete accessory styling.' },
        { id: 'back_veil_flow', label: '头纱飘逸', icon: Waves, color: '#E0F2FE', desc: '头纱展示', prompt: 'Back view emphasizing flowing wedding veil, veil spread out beautifully, wind effect or posed arrangement, ethereal and dreamy, perfect for veil product showcase.' },
    ]
};
...
const MODEL_STYLES = [
    ...
    { id: 'child_caucasian', label: '欧美儿童 (Caucasian Child 3-12y)' },
];
...
export const OperationPanel: React.FC = () => {
    ...
    const [genericReferenceImages, setGenericReferenceImages] = useState<string[]>([]); // New: Generic multi-reference
        ...
        if (files && files.length > 0) {
            Array.from(files).forEach((file) => {
                ...
                reader.onloadend = async () => {
                    ...
                    // Update reference images list
                    setGenericReferenceImages(prev => {
                        const newImages = [...prev, src];
                        // If this is the first image, set it as the main uploadedImage and add to canvas
                        if (!uploadedImage && prev.length === 0) {
                            setUploadedImage(src);
                            // Only add to canvas if it's the very first image
                            loadImageDimensions(src).then(({ width, height }) => {
               ...
               `.trim();
                    ...
                    try {
                        const imageUrl = await generateImage({
                            prompt: fullPrompt, negativePrompt: customNegative, aspectRatio, imageSize,
                            referenceImage: uploadedImage || undefined,
                            referenceImages: genericReferenceImages.length > 0 ? genericReferenceImages : undefined, // Check priority in service
                            poseImage: customPoseImage || (activeWorkflow === 'fission' && selectedPoses.length > 0 ? undefined : undefined), // Logic handled in service for poses
                            ...
                            [...layers].reverse().map((layer) => {
                                ...
                                return (
                                    <motion.div
                                        ...
                                        className={`flex items-center gap-3 p-3 rounded-lg border transition-all cursor-pointer group hover:shadow-sm ${isSelected
    ...
    // Helper to render the multi-image upload grid for generic workflows
    const renderGenericImageUpload = () => {
        return (
            <div className="space-y-3">
                <label className="text-sm font-medium text-studio-700 dark:text-studio-300 flex justify-between">
                    参考图 (Reference Images)
                    <span className="text-xs text-studio-400 font-normal">支持多图上传 ({genericReferenceImages.length})</span>
                </label>

                {genericReferenceImages.length > 0 ? (
                    <div className="grid grid-cols-3 gap-2">
                        {genericReferenceImages.map((img, index) => (
                            <div key={index} className="relative group aspect-square rounded-md overflow-hidden border border-studio-200 dark:border-studio-700">
                                <img src={img} alt={`Ref ${index}`} className="w-full h-full object-cover" />
                                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                    <button
                                        onClick={() => {
                                            const newImages = genericReferenceImages.filter((_, i) => i !== index);
                                            setGenericReferenceImages(newImages);
                                            // Update main image if we deleted the first one
                                            if (index === 0) {
                                                setUploadedImage(newImages.length > 0 ? newImages[0] : null);
                                            }
                                        }}
                                ...
                                {index === 0 && <span className="absolute top-1 left-1 text-[10px] bg-studio-900/70 text-white px-1.5 py-0.5 rounded">主图</span>}
                            </div>
                        ))}
                        <button
                            onClick={() => fileInputRef.current?.click()}
                            className="aspect-square flex flex-col items-center justify-center border-2 border-dashed border-studio-200 dark:border-studio-700 rounded-md hover:border-indigo-500 dark:hover:border-indigo-400 hover:bg-studio-50 dark:hover:bg-studio-800 transition-colors"
                        >
                            <Plus size={20} className="text-studio-400 mb-1" />
                            <span className="text-[10px] text-studio-400">添加</span>
                        </button>
                    </div>
                ) : (
                    <div
                        onClick={() => fileInputRef.current?.click()}
                        className="border-2 border-dashed border-studio-200 dark:border-studio-700 rounded-lg p-8 flex flex-col items-center justify-center cursor-pointer hover:border-indigo-500 hover:bg-indigo-50 dark:hover:border-indigo-500 dark:hover:bg-studio-800/50 transition-all group"
                    >
                        <div className="w-12 h-12 rounded-full bg-studio-100 dark:bg-studio-800 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
                            <Upload className="w-6 h-6 text-studio-400 group-hover:text-indigo-500 transition-colors" />
                        </div>
                        <p className="text-sm font-medium text-studio-600 dark:text-studio-300">上传产品或模特参考图</p>
                        <p className="text-xs text-studio-400 mt-1">支持多图上传，AI 将综合分析细节</p>
                    </div>
                )}
                ...
                {/* Main Image Preview & Tools (Keep existing analysis tool but base it on main image) */}
                {uploadedImage && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-2 p-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg">
                        <div className="flex items-start gap-3">
                            <Lightbulb className="w-4 h-4 text-indigo-600 dark:text-indigo-400 mt-1 flex-shrink-0" />
                            <div>
                                <h4 className="text-xs font-semibold text-indigo-900 dark:text-indigo-200 mb-1">AI 视觉分析 (基于主图)</h4>
                                <p className="text-xs text-indigo-700 dark:text-indigo-300 mb-2">分析主图细节以优化提示词。</p>
                                <button
                                    onClick={() => handleAnalyzeImage(uploadedImage)}
                                    className="text-xs bg-indigo-100 dark:bg-indigo-800 text-indigo-700 dark:text-indigo-200 px-2 py-1 rounded border border-indigo-200 dark:border-indigo-700 hover:bg-indigo-200 transition-colors"
                                >
                                    开始分析
                                </button>
                                {analysisResult && (
                                    <div className="mt-2 text-xs text-studio-600 dark:text-studio-400 bg-white/50 dark:bg-black/20 p-2 rounded">
                                        {analysisResult}
                                    </div>
                                )}
                            </div>
                ...
                {activeWorkflow !== 'agent_batch' && activeWorkflow !== 'extraction' && activeWorkflow !== 'planting' && activeWorkflow !== 'settings' && (
                    ...
                    <div className="space-y-3">
                        ...
                        {activeWorkflow === 'fusion' ? (
                            ...
                            // --- DEFAULT: GENERIC MULTI IMAGE UPLOAD (New) ---
                            renderGenericImageUpload()
                        )}
